...
export default class MainMenu {
  ...
  render(){
    ...
    if(!this._spriteGroup){
      ...
      const titleSprite = game.add.text(game.world.centerX, game.world.centerY-0.2*height, text.title, style.title);
      titleSprite.anchor.set(0.5);
      this._spriteGroup.add(titleSprite);

      const instructionSprite = game.add.text(game.world.centerX, game.world.centerY, text.instructions, style.instructions);
      instructionSprite.anchor.set(0.5);
      this._spriteGroup.add(instructionSprite);


      const startBtnSprite = this.sprite = game.add.sprite(game.world.centerX, game.world.centerY + 0.3 * height, `game_start_btn`);
      startBtnSprite.anchor.set(0.5);
      startBtnSprite.inputEnabled = true;
      startBtnSprite.events.onInputDown.add(() => {
